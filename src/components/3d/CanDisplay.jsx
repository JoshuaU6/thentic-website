...
// CSS 3D can that cycles through products
export default function HeroCan() {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 280, height: 400 }}>
      {/* Glow behind can */}
      <div
        className="absolute inset-0 rounded-full blur-3xl opacity-30 transition-all duration-1000"
        style={{ background: product.color, transform: 'scale(0.6)' }}
      />

      {/* The can */}
      <div
        className="relative transition-all duration-700 animate-float"
        style={{
          opacity: transitioning ? 0 : 1,
          transform: transitioning ? 'scale(0.9) rotateY(30deg)' : 'scale(1) rotateY(0deg)',
        }}
      >
        <CSSCan product={product} />
      </div>

      {/* Product name below can */}
      <div
      ...
      <div className="absolute -bottom-8 left-0 right-0 flex justify-center gap-2">
        {products.map((_, i) => (
          <button
            ...
            style={{ background: i === current ? '#C9A84C' : 'rgba(201,168,76,0.25)', transform: i === current ? 'scale(1.4)' : 'scale(1)' }}
          />
        ))}
      </div>
    </div>
...
function CSSCan({ product }) {
  return (
    ...
    >
      {/* Shine stripe */}
      <div style={{
        position: 'absolute', top: 0, left: '18%', width: '12%', height: '100%',
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 9, color: '#C9A84C', letterSpacing: '0.2em', marginBottom: 4, textAlign: 'center' }}>
          THENTIC MIX
        </div>
        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 11, fontWeight: 700, color: '#FAF5E9', textAlign: 'center', lineHeight: 1.3, letterSpacing: '0.06em' }}>
